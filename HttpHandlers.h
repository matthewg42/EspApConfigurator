...
const char HTTP_SCRIPT[] PROGMEM          = "<script>function c(l){document.getElementById('s').value=l.innerText||l.textContent;document.getElementById('p').focus();} function r(l){b=document.getElementById('r');b.innerHTML='scanning...';b.disabled=true;window.setTimeout(function(){console.log('redirect');window.location.href='';}, 3000);}</script>";
const char HTTP_HEAD_END[] PROGMEM        = "</head><body><div style='text-align:left;display:inline-block;min-width:260px;'>";
const char HTTP_ITEM[] PROGMEM            = "<div><a href='#p' onclick='c(this)'>{v}</a>&nbsp;<span class='q {i}'>{r}%</span></div>";
const char HTTP_FORM_START[] PROGMEM      = "<button id='r' onclick='r();'>Re-scan WiFi</button><h3>Wifi Settings</h3><p><label for='s'>SSID</label><input id='s' name='s' length=32><label for='p'>Passphrase</label><input id='p' name='p' length=64 type='password'><label for='h'>Hostname</label><input id='h' name='h' length=32 value='{h}'></p>";
